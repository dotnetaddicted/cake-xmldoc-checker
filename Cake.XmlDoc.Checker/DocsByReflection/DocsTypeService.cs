...
namespace Cake.XmlDoc.Checker.DocsByReflection
{
    ...
    internal static class DocsTypeService
    {
        ...
        public static XmlElement GetXmlForNamepace(string nameSpace, Type someAssemblyType, bool throwError)
        {
            const string prefix = "T";                      
            var fullName = String.Format(CultureInfo.InvariantCulture, "{0}:{1}.NamespaceDoc", prefix, nameSpace);
            var matchedElement = GetMatch(fullName, nameSpace, someAssemblyType, throwError);

            if (matchedElement == null)
            {
                fullName = String.Format(CultureInfo.InvariantCulture, "{0}:{1}.NamespaceGroupDoc", prefix, nameSpace);
                matchedElement = GetMatch(fullName, nameSpace, someAssemblyType, throwError);
            }
        ...
        private static XmlElement GetMatch(string fullName, string nameSpace, Type someAssemblyType, bool throwError)
        {
            XmlDocument xmlDocument = DocsService.GetXmlFromAssembly(someAssemblyType.Assembly, throwError);
            XmlElement matchedElement = null;
        ...
        public static XmlElement GetXmlForConstructor(Type type, string fullName, bool throwError)
        {
            XmlDocument xmlDocument = DocsService.GetXmlFromAssembly(type.Assembly, throwError);
            XmlElement matchedElement = null;
